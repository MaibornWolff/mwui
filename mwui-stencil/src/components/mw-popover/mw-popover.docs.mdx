...
A Popover can be used to display some content in relative position to an anchor element. Internally it is used for `menus` and `dropdowns`.

<Canvas withSource="open">
  <Story id="components-popover--default" />
</Canvas>

<ArgsTable of="mw-popover" />
...
<Canvas>
  <div>
    ...
    <div style={{ height: '150px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '50px' }}>
      {['top-start', 'bottom-start', 'bottom-end', 'bottom-end'].map(placement => (
        <mw-popover key={placement} placement={placement}>
          <mw-button slot="anchor" label={placement} variant="outline"></mw-button>
          <div slot="content">Popover {placement}</div>
        </mw-popover>
...
<Canvas withSource="open">
  <Story id="components-popover--dismissable" />
</Canvas>
