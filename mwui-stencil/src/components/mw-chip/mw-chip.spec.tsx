...
describe("GIVEN MwChip", () => {
  const setup = async (
    { icon, disabled, showClose, selected }: Pick<MwChip, "icon" | "disabled" | "showClose" | "selected"> = {
      selected: false,
    },
    text = "",
  ) => {
    return await newSpecPage({
      components: [MwChip],
      template: () => (
        <mw-chip icon={icon} disabled={disabled} showClose={showClose} selected={selected}>
          {text}
        </mw-chip>
      ),
    });
  ...
  const clickCloseButton = (page: SpecPage) => {
    page.root.shadowRoot.querySelector(".chip").querySelector<HTMLButtonElement>("span").click();
  };

  const toggleChip = (page: SpecPage) => {
    page.root.shadowRoot.querySelector<HTMLButtonElement>(".chip").click();
  };
